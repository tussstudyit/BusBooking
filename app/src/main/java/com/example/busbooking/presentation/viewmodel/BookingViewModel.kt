...
class BookingViewModel(
    ...
    fun bookTicket(tripId: Long) {
        ...
        if (userId == null) {
            _bookingState.value = BookingState.Error("Phiên đăng nhập hết hạn, vui lòng đăng nhập lại")
            return
        }
        ...
        viewModelScope.launch {
            when (val result = ticketRepository.bookTicket(userId, tripId, seat.id)) {
                is BookingResult.Success -> {
                    _bookingState.value = BookingState.BookingSuccess(result.ticketId)
                }
                is BookingResult.AlreadyBooked -> {
                    _bookingState.value = BookingState.Error("Ghế này vừa được người khác đặt, vui lòng chọn ghế khác")
                }
                is BookingResult.InvalidSeat -> {
                    _bookingState.value = BookingState.Error("Ghế không hợp lệ")
                }
                is BookingResult.Failure -> {
                    _bookingState.value = BookingState.Error("Đặt vé thất bại, vui lòng thử lại")
                }
            }
        }
    }
